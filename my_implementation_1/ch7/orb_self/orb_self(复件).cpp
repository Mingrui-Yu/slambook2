...
//     // 读取图像
//     cv::Mat img_1 = cv::imread("../1.png", CV_LOAD_IMAGE_COLOR);
//     cv::Mat img_2 = cv::imread("../2.png", CV_LOAD_IMAGE_COLOR);
//     assert(img_1.data != nullptr && img_2.data != nullptr);


//     //检测FAST角点，并计算Oriented FAST角点
//     chrono::steady_clock::time_point t1 = chrono::steady_clock::now();

//     vector<cv::KeyPoint> keypoints1, keypoints2;
//     cv::FAST(img_1, keypoints1, 40);
//     cv::FAST(img_2, keypoints2, 40);
//     vector<DescType> descriptor1, descriptor2;
    
//     ComputeORB(img_1, keypoints1, descriptor1);
//     ComputeORB(img_2, keypoints2, descriptor2);

//     chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
//     chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>(t2-t1);
//     cout << "extract ORB cost = " << time_used.count() << endl;

//     cv::Mat outimg1;
//     drawKeypoints(img_1, keypoints1, outimg1, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT);
...
//     // plot the matches
//     cv::Mat image_show;
//     cv::drawMatches(img_1, keypoints1, img_2, keypoints2, matches, image_show);
//     cv::imshow("matches", image_show);
//     cv::imwrite("matches.png", image_show);
//     cv::waitKey(0);
...
//         // 去除边界边上的点
//         if (kp.pt.x < half_boundary || img.cols - kp.pt.x <= half_boundary 
//             || kp.pt.y < half_boundary || img.rows - kp.pt.y <= half_boundary){
//                 bad_points++;
//                 descriptors.push_back({});
//                 continue;
//             }
...
//             for (int k=0; k<32; k++){
//                 int idx_pq = 32 * i + k;
//                 cv::Point2f p = cv::Point2f(ORB_pattern[4*idx_pq, 4*idx_pq+1]);
//                 cv::Point2f q = cv::Point2f(ORB_pattern[4*idx_pq+2, 4*idx_pq+3]);

//                 cv::Point2f pp = kp.pt + cv::Point2f(p.x*cos_theta - p.y*sin_theta, p.y*cos_theta + p.x*sin_theta);
//                 cv::Point2f qq = kp.pt + cv::Point2f(q.x*cos_theta - q.y*sin_theta, q.y*cos_theta + q.x*sin_theta);

//                 if (img.at<char>(pp.y, pp.x) < img.at<char>(qq.y, qq.x)){
//                     d |= 1<<k;
//                 }
//             }
//             desc[i] = d;
