...
int main(int argc, char **argv){
    ...
    for (int index = 1; index < 2; index++){
        ...
        SE3d pose_T_C_R = pose_curr_TWC.inverse() * pose_ref_TWC; //两个相机之间的相对位姿
        ...
        // imwrite("depth_2.png", depth);
    }

    cout << "estimation returns, saving depth map ..." << endl;
    imshow("depth", depth);
    waitKey(0);
    imwrite("depth_2.jpg", depth * 255);
    cout << "done." << endl;
...
bool updateDepthFilter(
    ...
    double alpha = acos(p.dot(t) / (t_norm * p.norm()));
    double beta = acos(-a.dot(t) / (a_norm * t_norm));
    Vector3d f_curr_prime = px2cam(pt_curr + epipolar_direction);
    // f_curr_prime.normalize();
    double beta_prime = acos(f_curr_prime.dot(-t) / (t_norm * f_curr_prime.norm()));
