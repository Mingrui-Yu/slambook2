...
bool Frontend::Track(){
    ...
    TrackLastFrame();  // int num_track_last = TrackLastFrame();
...
int Frontend::TrackLastFrame(){
    ...
    for(size_t i = 0; i < status.size(); ++i){
        if (status[i]){
            cv::KeyPoint kp(kps_current[i], 7);  // ??? 7是什么
...
int Frontend::FindFeaturesInRight(){
    ...
    cv::calcOpticalFlowPyrLK(current_frame_->left_img_,
        current_frame_->right_img_, kps_left, kps_right, status, error, cv::Size(11, 11), 3,
        cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 30, 0.01),
        cv::OPTFLOW_USE_INITIAL_FLOW);
